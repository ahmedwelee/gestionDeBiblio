...
export default function BorrowRecordAddForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://localhost:8000/api/borrow-records/create', {
        book_id,
        user_id,
        borrow_date,
        due_date,
      ...
      setError({});
      window.location.href = "/admin/dashboard/borrow-records?message=Borrow record added successfully";
    } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="backdrop-blur-xl bg-white/60 shadow-xl rounded-3xl p-8 border border-gray-100">
          <header className="flex justify-between items-center mb-8 border-b border-gray-100 pb-6">
            <div>
              ...
              <p className="text-gray-500 mt-1">Create a new borrowing record</p>
            </div>
            <Link 
              to="/admin/dashboard/borrow-records" 
          ...
          <datalist id="users">
            {filteredUsers.map(user => (
              ...
              <div className="relative">
                <label htmlFor="borrow_date" className="block text-sm font-medium text-gray-700 mb-1">Borrow Date</label>
                 <div className="relative group">
                   <div className="absolute -inset-0.5 bg-gradient-to-r from-brand-primary to-brand-primary-dark rounded-lg blur opacity-0 group-hover:opacity-20 transition duration-200"></div>
           <input
             type="date"
           ...
           />
                 ...
                 {error.borrow_date && <p className="text-red-500 dark:text-red-400 text-xs mt-1">{error.borrow_date}</p>}
         </div>
           ...
           />
                ...
                {error.due_date && <p className="text-red-500 text-xs mt-1">{error.due_date}</p>}
        </div>
      </div>
